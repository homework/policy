...
sqlfilter *sqlstmt_new_filter(int ctype, char *name, int dtype, char *value) {
	...
	switch(ctype) {
		...
		case CONTAINS:
		filter->sign = SQL_FILTER_CONTAINS; break;
		case NOTCONTAINS:
		filter->sign = SQL_FILTER_NOTCONTAINS; break;
	}
	switch(dtype) {
	    case INTEGER:
		filter->value.intv = strtoll(value, NULL, 10); break;
	    case REAL:
...
sqlpair *sqlstmt_new_pair(int ctype, char *name, int dtype, char *value) {
	...
	pair = mem_alloc(sizeof(sqlpair));
	pair->IS_STR = 0;
	pair->varname = name;
	switch(ctype) {
	    case EQUALS:
		pair->sign = SQL_PAIR_EQUAL; break;
	    case ADD:
		pair->sign = SQL_PAIR_ADDEQ; break;
	    case SUB:
		pair->sign = SQL_PAIR_SUBEQ; break;
	}
	switch(dtype) {
	    case INTEGER:
		pair->value.intv = strtoll(value, NULL, 10); break;
	    case REAL:
		pair->value.realv = strtod(value, NULL); break;
	    case CHARACTER:
		pair->value.charv = value[0]; break;
	    case TINYINT:
		pair->value.tinyv = atoi(value) & 0xff; break;
	    case SMALLINT:
		pair->value.smallv = atoi(value) & 0xffff; break;
	    case TSTAMP:
		pair->value.tstampv = string_to_timestamp(value); break;
	    case VARCHAR:
